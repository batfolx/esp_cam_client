...
void sendPhoto() {
  ...
  if ( c.connect (host.c_str(), port) ) {
    ...
    c.println("Host: " + host + ":" + port);
    c.println("Content-Length: " + String(buf_len));
    c.println("Content-Type: image/jpeg");
    c.println("Content-Disposition: 3.jpg");
    c.println();
  
    int chunksize = 1024;
    uint8_t * ptr = fb->buf;
    for (int i = 0; i < buf_len; i += chunksize) {
  
      if ( i + chunksize < buf_len ) {
        c.write(ptr, chunksize);
        ptr += chunksize;
      } else if ( buf_len % 1024 > 0 ) {
        c.write(ptr, buf_len % 1024);
      }
